#Import and define all the necessities
#----------------------------------------
#Import libraries
import cv2, os
import numpy as np
...
# Set constants: Define objectness, prob and NMS thresholds
OBJ_THRESH = .6
P_THRESH = .6
NMS_THRESH = .5
...
#Import the configuration and weight files for the model
cfg = 'yolov3/yolov3.cfg'
weights = 'yolov3/yolov3.weights'

# Load the next work using them
...
def detectObjectCoord(frame, outputs):
	#frame: video
	#outputs: corresponding YOLO output
	#returns: bounding box coordinates, class probabilities and labels of the predictions (class id)
...
# Create new window to process the new video
...
while(vid.isOpened()):
    ...
    #read video
    ok, frame = vid.read()
    #If no high-confidence boxes are predicted we repeat detection in the next frame
    #Otherwise, if tracking fails we switch back to detection.
    if ok:
        if perform_detection:
            ...
            bboxes, probs, class_ids = detectObjectCoord(frame, outputs)
            
            #if there are detected boundary boxes
            if len(bboxes) > 0:
                # Init multitracker
                ...
                for i in idxs:
                    ...
                    cv2.putText(frame, 'Detecting now', (20, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0,0,255), 2)

                # Increase counter
                count += 1

            else: # declare failure
                cv2.putText(frame, 'Detection failed', (20, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0,0,255), 2)
            ...
            if is_tracking:
                for i, bbox in enumerate(bboxes):
                    ...
                    #only track the soccer ball
                    if class_id=='sports ball':
	                    # Mark tracking frame with corresponding color, write class name on top
	                    cv2.rectangle(frame, (x, y), (x+w, y+h), col, 2)
	                    cv2.putText(frame, class_id, (x, y - 15),cv2.FONT_HERSHEY_SIMPLEX, 0.5, col, 2)
	                    cv2.putText(frame, 'Tracking now', (20, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0,0,255), 2)
                # Increase counter
                count += 1
